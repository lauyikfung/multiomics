...
def json_to_xlsx(filename : str, dest_name = "result.xlsx"):
  ...
  with open(filename, "r") as f:
    li = json.load(f)
  res = pd.DataFrame(columns=("order", "total", "compound", "gene", "ortholog", "Non-reaction","reaction"))
  for i in range(len(li)):
    res.loc[i + 1] = [li[i]["order"],li[i]["total"],li[i]["compound"],li[i]["gene"],li[i]["ortholog"],li[i]["total"]-li[i]["reaction"],li[i]["reaction"]]
